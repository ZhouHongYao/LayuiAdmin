  ...
  .define(
    ...
    function(exports) {
      ...
      self.loading = function(elem) {
        elem.append(
          (this.elemLoad = $(
            '<i class="layui-anim layui-anim-rotate layui-anim-loop layui-icon layui-icon-loading layuiAdmin-loading"></i>'
          ))
        )
      }
      ...
      self.tab = {
        ...
        minLeft: null,
        maxLeft: null,
        wrap: '.layuiAdmin-tabs-wrap',
        menu: '.layuiAdmin-tabs-menu',
        next: '.layuiAdmin-tabs-next',
        prev: '.layuiAdmin-tabs-prev',
        step: 200,
        init: function() {
          var tab = this
          var btnCls = tab.wrap + ' .layuiAdmin-tabs-btn'

          layui.dropdown.render({
            elem: '.layuiAdmin-tabs-down',
            click: function(name) {
          ...
          $(document).on('click', btnCls, function(e) {
            ...
            if ($(e.target).hasClass('layuiAdmin-tabs-close')) {
              tab.del(url)
            } else {
          ...
          $('.layuiAdmin-tabs-hidden').addClass('layui-show')
          this.isInit = true
        },
        has: function(url) {
          var exists = false
        ...
        delAll: function(type) {
          var tab = this
          var menuBtnClas = tab.menu + ' .layuiAdmin-tabs-btn'
        ...
        delOther: function() {
          var tab = this
          var menuBtnClas = tab.menu + ' .layuiAdmin-tabs-btn'
          $(menuBtnClas + '.layuiAdmin-tabs-active')
            .siblings()
            .each(function() {
              var url = $(this).attr('lay-url')
              tab.del(url)
        ...
        del: function(url, backgroundDel) {
          ...
          var lay = '[lay-url="' + url + '"]'
          var thisBody = $(
            '#' + conf.containerBody + ' > .layuiAdmin-tabs-item' + lay
          )
          ...
          if (backgroundDel === undefined) {
            if (thisMenu.hasClass('layuiAdmin-tabs-active')) {
        ...
        clear: function() {
          this.data = []
          this.isInit = false
          $(document).off('click', this.wrap + ' .layuiAdmin-tabs-btn')
        },
        change: function(route, callback) {
          ...
          var activeCls = 'layuiAdmin-tabs-active'
          ...
          if (existsTab) {
            ...
            self.loadHtml(fileurl, function(res) {
              var htmlElem = $(
                "<div><div class='layuiAdmin-tabs-item' lay-url='" +
                  fileurl +
                  "'>" +
                  res.html +
                  '</div></div>'
        ...
        if (tab.change(data.fileurl) === false) {
          ...
          if (data) {
            ...
            thisMenu
              .addClass('layuiAdmin-tabs-active')
              .siblings()
              .removeClass('layuiAdmin-tabs-active')
            this.minLeft = this.minLeft || parseInt(menu.css('left'))
            ...
            $('#' + conf.containerBody + ' > .layuiAdmin-tabs-item' + layUrl)
              .show()
              .siblings()
              .hide()
            $(document).scrollTop(-100)
      ...
      self.fillHtml = function(url, htmlElem, modeName) {
        ...
        //重新对面包屑进行渲染
        layui.element.render('breadcrumb', 'layuiAdmin-breadcrumb')
      ...
      self.renderLayout = function(callback, url) {
        ...
        self.render(url, function(res) {
          ...
          if (conf.viewTabs == true) {
            self.containerBody.addClass('layuiAdmin-tabs-body')
          }
